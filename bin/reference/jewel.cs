...
public class GameController : MonoBehaviour {
	...
	public enum BState {  						//State of the Board
			PLAYING,
			DROP_JEWELS,
			SWAPPING,
			GAMEOVER,
	...
	public static int CurrentlyMovingJewels; 	//Controls if the Jewels are moving. Controlled by Jewel.cs
	...
	private List<GameObject> jewelRemains;		//List of jewels used to update when a jewel is destroyed or remains	
	private bool needToDropJewels;				//Controls the Dropping Jewels
	private int totalRemovedJewels;				//Controls how many jewels were removed in the game, you can use as Score 
	...
	private bool needToCheckCascades = false;	//Check for cascades
	private bool mouseLeftDown = false;			//Check for mouseUp
	private int totalTriplets = 0;				//Counter for triplets
	
	public GameObject Rock;						//Can be used as Rock or special item 
	public GameObject[] jewels;					//That's the jewels used on the game needs to have at least 3
	public static GameObject[,] jewelMapPosition;  //Hold's the jewels virtual position 
					
		private struct theSwap {				//helper for the swap I grab this part of the code from some websites
			...
			public void Init() {				//Resets the variables above to restart the swap
				jewelAx = jewelAy = -1;	
				jewelBx = jewelBy = -1;
				isActive = false;
				twiceClick = false;
		...
		private float delayDrop;				// Controls the delay of the dropped jewels
	
		private List<GameObject> hintJewels;	//controls the Hint Jewels	
		...
		private int skullInitial = 100;  		//how many points the skull will reduce
	
		public Texture2D flash;					//Texture for the Screen Flash
		public static bool flashIt;				//Flashes the Screen if true
		private float aFlash = 1;				//Alpha that controls the flash timer
	...
	private void ResetJewels() {
			...
			for (y = 0; y < boardSize; y++) {
				for (x = 0; x < boardSize; x++) {
					// Avoid a group of three
					GameObject objectType1 = jewels[UnityEngine.Random.Range(0,jewels.Length)];
	...
	public void DestroyJewelType(string tagType,int howMuch){
			...
			if(isRemoveJewel > 0){  //Was something removed?
	...
	void Update () {   
		...
		if(theBoardState == BState.RESETING) return; //Called when the board is being reseted.
		...
		if (CurrentlyMovingJewels > 0) return;
			...
			// Reduce the jewel dropping delay 
			if (delayDrop > 0) {
				delayDrop -= (float)Time.deltaTime;
				return;
			}
			// Game state
			if (CurrentlyMovingJewels > 0) theBoardState = BState.DROP_JEWELS;
			else {
				if (needToCheckCascades) {
					CheckForCascades();
					needToCheckCascades = false;
				}
				if (validateSwap) { 			// Validate the new position for jewels after swap
			...
			if (theBoardState == BState.PLAYING) { 	// Increase the hint timer if there's no action on the board
				hintTimer += (float)Time.deltaTime;
				if (hintTimer >= hintDelay)			// Reset the hint delay
					ShowRandomHint();
			}
		
		    if(Input.GetMouseButton(0)){ 			// Check if there is a touch or mouse on some tile
			...
			if (canSwap && mouseClickX > -1 && Input.GetMouseButton(0)) { // Start to swap if some jewel is clicked
			...
			if (canSwap && Input.GetMouseButton(0)) { // When the mouse button or touch is released..
				mouseLeftDown = false;
				if(tip){ 
				  tip.alpha = false;
				  tip = null;
				...
				if (theBoardState == BState.SWAPPING) {
					...
					} else if (!IsLegalSwap()) { 			//Cancels Swap
						swapping.isActive = false;
						theBoardState = BState.PLAYING;
					//SelectedJewel.transform.localPosition = new Vector3(-6,-6,-1.02f);
					} else if (!swapping.twiceClick) { 		//Swap
						audio.pitch = 1;
			...
			if (needToDropJewels && delayDrop <= 0) { // Check for drop jewels
				needToDropJewels = false;
	...
	private void SwapJewels(theSwap swap) {
			...
			if(!verifyRock) { //Handles the swap
	...
	public void CheckForFallingJewels() {
			for (int x = 0; x < boardSize; x++) {			// Checks every column from left to right, from bottom to up
				int totalSlots = 0;
				for (int y = boardSize - 1; y > 0; y--) {	// search for holes
					if (!IsJewelAt(x, y)) { 				// if there is a hole then check how tall it is
			...
			for (int x = 0; x < boardSize; x++) {			// Check the top row for empty slots
				if (!IsJewelAt(x, 0)) {
					int slots = HowBigHoleAt(x, 0, false);	// if there is a hole then check how tall it is

					for (int y = 0; y < slots; y++) {		// Create a new jewel, position it out of the screen to fall down
	...
	public void CheckForCascades() {
			int total = RemoveTriplets();
			if (total == 0) { 						// No more cascades
				totalTriplets = 0;
				canSwap = true;

				movesLeft = HowManyMovesLeft();		// Calculate the number of moves

				if (movesLeft == 0) {				// Use this if you want to gameOver when there are no moves left
						PopulateWithNewJewels();	// In this case I am repopulating with new jewels
				}
			}
	}
	...
	public int CountMatch3() {
			...
			for (int y = 0; y < boardSize; y++) { 		// Horizontal
				int counter = 1;
			...
			for (int x = 0; x < boardSize; x++) {		// Vertical
				int counter = 1;
	...
	private void ScoreMatch3(int jewels, int start, int end, int lineRow, bool horizontal) {
			...
			if (horizontal) {
				...
				if(foundSkull) PointsAni(-skullInitial,new Vector2(start+1,lineRow)); else PointsAni(tripletScore,new Vector2(start+1,lineRow));
			} else {
			...
			if(totalTriplets > 1 && !foundSkull && !GameObject.FindGameObjectWithTag("MMsg")){
				...
				//Loads the Textures
				if(totalTriplets == 2) msg.renderer.material.mainTexture = (Texture2D)Resources.Load("Textures/2Dtextures/P1");
				if(totalTriplets == 3) msg.renderer.material.mainTexture = (Texture2D)Resources.Load("Textures/2Dtextures/P2");
				if(totalTriplets == 4) msg.renderer.material.mainTexture = (Texture2D)Resources.Load("Textures/2Dtextures/P3");
				if(totalTriplets >= 5) msg.renderer.material.mainTexture = (Texture2D)Resources.Load("Textures/2Dtextures/P4");
			}
			...
			if(foundSkull && !GameObject.FindGameObjectWithTag("MMsg")){
				...
				skullmsg.renderer.material.mainTexture = (Texture2D)Resources.Load("Textures/2Dtextures/P5");
			}
		...
		switch(item)       
         ...
         default:  
            ...
            break;      
				...
				if(item == "Item6"){  //if Skull
					audio.PlayOneShot(explodeSound);
			 		CameraShake.shakeFor(0.5f,0.1f); 
				}else{
				 	audio.PlayOneShot(match3Sound);
				}
			
				//Example
				//if(itemCh == 0) print("you matched"+ jewels);
	...
	private int RemoveTriplets() {
			...
			if (triplets == 0) return 0;			// Nothing to remove
	...
	void OnGUI(){
		if(flashIt){ //Just flashes the Screen
			aFlash -= 0.01f;
	...
	//This function calls the prefab that shows the points going up on the screen
	void PointsAni(int a,Vector2 position){
		...
		if(a > 0) msg2.renderer.material.color = Color.green;
		if(a < 0) msg2.renderer.material.color = Color.red;
		m.direction = 1;

    ...
    int       nCmdShow)
{
    ...
    return EliminatePlayer::createAndRun();
}

int EliminatePlayer::createAndRun(void)
{
    EliminatePlayer *host = EliminatePlayer::sharedInstance();
    int ret = host->run();
    EliminatePlayer::purgeSharedInstance();
    return ret;
}

EliminatePlayer *EliminatePlayer::s_sharedInstance = NULL;

EliminatePlayer *EliminatePlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new EliminatePlayer();
    }
    return s_sharedInstance;
}

void EliminatePlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
EliminatePlayer::EliminatePlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int EliminatePlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "eliminate-player");
...
void EliminatePlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void EliminatePlayer::createViewMenu(void)
{
...
void EliminatePlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void EliminatePlayer::updateWindowTitle(void)
{
}

void EliminatePlayer::relaunch(void)
{
...
void EliminatePlayer::writeDebugLog(const char *log)
{
...
// menu callback
void EliminatePlayer::onFileOpenProject(void)
{
...
void EliminatePlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"EliminatePlayer");
...
void EliminatePlayer::onFileProjectConfig(void)
{
...
void EliminatePlayer::onFileRelaunch(void)
{
    relaunch();
}

void EliminatePlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void EliminatePlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void EliminatePlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void EliminatePlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void EliminatePlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT EliminatePlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    EliminatePlayer *host = EliminatePlayer::sharedInstance();
...
INT_PTR CALLBACK EliminatePlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
